...
// LNB baseline은 `lnb-` 접두 qa-id만 필터
...
test.describe('LNB QA — Figma vs Browser', () => {
  test.beforeAll(async () => {
    if (!fs.existsSync(DIFF_DIR)) fs.mkdirSync(DIFF_DIR, { recursive: true });
  });

  if (availableIds.length === 0) {
    test('baseline 이미지 없음 — tests/figma-baseline/lnb-*.png 추가 시 활성화', async () => {
      // figma-qa-map.ts 참조 후 lnb- 접두 PNG 추가하면 자동 실행됨
    });
    return;
  }
